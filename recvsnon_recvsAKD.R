...
contrast_matrix <- rbind(
  "Non-Recovery vs nonAKD"     = nonrec_vec - nonAKD_vec,
  "Recovery vs nonAKD"         = rec_vec - nonAKD_vec,
  "Recovery vs Non-Recovery"= rec_vec - nonrec_vec
)

contrast_test <- glht(fit_window, linfct = contrast_matrix, vcov = vcov(fit_window))

# ????信頼区間は明示的に呼ぶ必要あり！
contrast_conf <- confint(contrast_test)
contrast_summary <- summary(contrast_test)

# 正常な data.frame に変換
contrast_df <- data.frame(
  Comparison = names(contrast_summary$test$coefficients),
  Difference = as.numeric(contrast_summary$test$coefficients),
  CI_Lower = contrast_conf$confint[, "lwr"],
  CI_Upper = contrast_conf$confint[, "upr"],
  p_value = contrast_summary$test$pvalues
)
...
y_min <- floor(min(all_df$CI_Lower, na.rm = TRUE)) - 0.5
y_max <- ceiling(max(all_df$CI_Upper, na.rm = TRUE)) + 0.5

# 描画用の共通関数（順序・配色・軸・テーマを統一）
plot_slope <- function(df, main_title){
  df <- dplyr::mutate(
    df,
  ...
  ggplot(df, aes(x = Group, y = Estimate, fill = Group)) +
    ...
    labs(x = NULL, y = "Slope (95% CI)", title = main_title) +
    theme(legend.position = "none")
}

# 各期間のプロット
p1 <- plot_slope(slope_df_1year, "Estimated eGFR Slopes by Group (1year_period)")
p2 <- plot_slope(slope_df_2year, "Estimated eGFR Slopes by Group (2year_period)")
p3 <- plot_slope(slope_df_3year, "Estimated eGFR Slopes by Group (3year_period)")
p4 <- plot_slope(slope_df_full,  "Estimated eGFR Slopes by Group (full period)")

# 個別に表示（必要なものだけ print でもOK）
p1; p2; p3; p4

# 4枚を同一キャンバスに並べたい場合（任意）
...
#index_date+210から半年の死亡を除外した死亡のカプランマイヤー#####
# 前処理：一意のidを抽出（そのまま）
jin1_Eligibile_unique_id <- jin1_Eligibile %>%
  dplyr::group_by(id) %>%
  dplyr::arrange(index_date) %>%
  dplyr::slice(1) %>%
  dplyr::ungroup()
...
# 半年を日数で定義（365.25/2 ≒ 182.625日）
six_months_days <- 365.25 / 2

# 死亡のKM用データ（半年以内死亡を除外）
jin1_Eligibile_death_6month_delete <- jin1_Eligibile_unique_id %>%
  mutate(
    jin_label = case_when(
      jin_status == "nonAKD" ~ "nonAKD",
      jin_status == "AKD" & `150_210recovery` == 1 ~ "Recovery",
  ...
  # ラベル欠損は除外
  filter(!is.na(jin_label)) %>%
  # 「index_plus_210 以降6か月以内の死亡」を除外
  # （death_date が NA なら除外しない）
  filter(
    !(
      !is.na(death_date) &
        death_date >= index_plus_210 &
        as.numeric(death_date - index_plus_210) <= six_months_days
    )
  ) %>%
  # 解析用の追跡時間（年）
  mutate(time_years = as.numeric(last_follow_death - index_plus_210) / 365.25) %>%
  # 負の追跡は除外
  filter(time_years >= 0)

# カプランマイヤー
fit_death <- survfit(Surv(time_years, primary_death) ~ jin_label, data = jin1_Eligibile_death_6month_delete)

ggsurvplot(
  fit_death, data = jin1_Eligibile_death_6month_delete,
  fun = "event",               # 1 - S(t) = 累積死亡率
  pval = TRUE, conf.int = TRUE,
  risk.table = TRUE,
  xlim = c(0, 10),             # 10年まで表示
  ylim = c(0, 0.4),
  title = "death (excluding deaths within 6 months after index_date + 210 days)",
  xlab = "year",
  ylab = "event"
)
#time0から半年の死亡を除外した死亡のカプランマイヤー#####
# 前処理：一意のidを抽出（そのまま）
jin1_Eligibile_unique_id <- jin1_Eligibile %>%
  dplyr::group_by(id) %>%
  dplyr::arrange(index_date) %>%
...
# 1) jin1_inclusion から idごとに一意の time0 を作成（最も早い time0 を採用）
time0_by_id <- jin1_inclusion %>%
  select(id, time0) %>%                 # 必要列だけ
  filter(!is.na(time0)) %>%             # 欠損を除外
  arrange(id, time0) %>%                # 早い順に整列
  group_by(id) %>%
  slice_head(n = 1) %>%                 # 各 id で先頭 (=最も早い time0)
  ungroup()
# 2) jin1_Eligibile_unique_id に突合
jin1_Eligibile_unique_time0 <- jin1_Eligibile_unique_id %>%
  left_join(time0_by_id, by = "id")
...
# 死亡のKM用データ（半年以内死亡を除外）
jin1_Eligibile_death_time0_6month_delete <- jin1_Eligibile_unique_time0  %>%
  mutate(
    jin_label = case_when(
      jin_status == "nonAKD" ~ "nonAKD",
      jin_status == "AKD" & `150_210recovery` == 1 ~ "Recovery",
...
# カプランマイヤー
fit_death <- survfit(Surv(time_years, primary_death) ~ jin_label, data = jin1_Eligibile_death_time0_6month_delete)

ggsurvplot(
  fit_death, data = jin1_Eligibile_death_time0_6month_delete,
  fun = "event",               # 1 - S(t) = 累積死亡率
  pval = TRUE, conf.int = TRUE,
  risk.table = TRUE,
  xlim = c(0, 10),             # 10年まで表示
  ylim = c(0, 0.4),
  title = "death (excluding deaths within 6 months after time0)",
  xlab = "year",
  ylab = "event"
)
...
#time0から半年の死亡を除外したeGFRスロープ#####
library(dplyr)
library(nlme)
# library(survival); library(survminer) は不要なので省略

## 0) 半年の閾値（日）
six_months_days <- 365.25 / 2  # ≒182.625日

## 1) time0から半年以内に死亡した患者を除外
#   - death_date が NA の症例は除外しません
#   - death_date - time0 が 0～182.6日以内の症例を除外します
akd_time_death_delete <- jin1_inclusion %>%
  mutate(days_from_time0_to_death = as.numeric(death_date - time0)) %>%
  filter(!( !is.na(days_from_time0_to_death) &
              days_from_time0_to_death >= 0 &
              days_from_time0_to_death <= six_months_days ))

## 2) Time Window 用の軸（years_from_time0）を作成（除外後データで作成）
akd_time_death_delete <- akd_time_death_delete %>%
  mutate(
    years_from_time0 = as.numeric(date - time0) / 365.25  # 年単位
  ) %>%
  filter(years_from_time0 >= 0)

## 3) 測定間隔の中央値
measurement_intervals <- akd_time_death_delete %>%
  arrange(id, years_from_time0) %>%
  group_by(id) %>%
  mutate(interval = years_from_time0 - dplyr::lag(years_from_time0)) %>%
  filter(!is.na(interval) & interval > 0) %>%
  ungroup()

median_interval <- median(measurement_intervals$interval, na.rm = TRUE)

## 4) 規定時点（target_timepoints）の設定
time_range <- range(akd_time_death_delete$years_from_time0, na.rm = TRUE)
...
cat("規定時点:", paste(target_timepoints, collapse = ", "), "\n")

## 5) ウィンドウ幅（中央値×0.75を採用）
window_width <- median_interval * 0.75

## 6) Time Window データ作成関数（変更なし）
create_time_window_data <- function(data, targets, window) {
  result_list <- vector("list", length(targets))
  for (i in seq_along(targets)) {
    target <- targets[i]
...
## 7) Time Window データの生成（除外後データで）
window_data <- create_time_window_data(akd_time_death_delete, target_timepoints, window_width)

## 8) lme用の time0_egfr を付与
#   years_from_time0 が 0 に最も近い観測の egfr を time0_egfr とする
akd_time_death_delete <- akd_time_death_delete %>%
  arrange(id, abs(years_from_time0)) %>%
  group_by(id) %>%
  mutate(time0_egfr = egfr[which.min(abs(years_from_time0))]) %>%
  ungroup()

## 9) 線形混合モデル（1年）を除外後データで実行
fit_window_within_1year_death_delete <- nlme::lme(
  egfr ~ years_from_time0 * jin_label + time0_egfr - 1,
  random = list(id = nlme::pdSymm(form = ~ 1 + years_from_time0)),
  na.action = na.omit,
  data = dplyr::filter(akd_time_death_delete, years_from_time0 <= 1),
  method = "REML",
...
# 0) 下ごしらえ：years_from_time0 と time0_egfr の付与（安全運転で再計算）
akd_time_dd <- akd_time_death_delete %>%
  mutate(
...
fit_window_within_1year_death_delete <- nlme::lme(
  ...
  random = list(id = nlme::pdSymm(form = ~ 1 + years_from_time0)),
  na.action = na.omit,
  data = dplyr::filter(akd_time_dd, years_from_time0 <= 1),
  method = "REML",
  control = ctrl
)

fit_window_within_2year_death_delete <- nlme::lme(
  egfr ~ years_from_time0 * jin_label + time0_egfr - 1,
  random = list(id = nlme::pdSymm(form = ~ 1 + years_from_time0)),
  na.action = na.omit,
  data = dplyr::filter(akd_time_dd, years_from_time0 <= 2),
  method = "REML",
  control = ctrl
)

fit_window_within_3year_death_delete <- nlme::lme(
  egfr ~ years_from_time0 * jin_label + time0_egfr - 1,
  random = list(id = nlme::pdSymm(form = ~ 1 + years_from_time0)),
  na.action = na.omit,
  data = dplyr::filter(akd_time_dd, years_from_time0 <= 3),
  method = "REML",
  control = ctrl
)

fit_window_death_delete <- nlme::lme(
  egfr ~ years_from_time0 * jin_label + time0_egfr - 1,
  random = list(id = nlme::pdSymm(form = ~ 1 + years_from_time0)),
  na.action = na.omit,
  data = akd_time_dd,  # 全期間
  method = "REML",
  control = ctrl
)

# 2) スロープ抽出ヘルパー（ご提示版＋微調整）
extract_slope_df <- function(fit, horizon_label){
  cn <- names(fixef(fit))
  p  <- length(cn)
  ...
  # 相互作用の係数名（Non-Recovery / Non.Recovery の両対応）
  idx_rec    <- which(cn == "years_from_time0:jin_labelRecovery")
  idx_nonrec <- which(cn %in% c("years_from_time0:jin_labelNon-Recovery",
                                "years_from_time0:jin_labelNon.Recovery"))
...
# 3) 4モデルからデータ結合
slope_all <- dplyr::bind_rows(
  extract_slope_df(fit_window_within_1year_death_delete, "within1 year"),
  extract_slope_df(fit_window_within_2year_death_delete, "within2 years"),
  extract_slope_df(fit_window_within_3year_death_delete, "within3 years"),
  extract_slope_df(fit_window_death_delete,              "All period")
) %>%
  mutate(
    Horizon = factor(Horizon,
                     levels = c("within1 year","within2 years","within3 years","All period"))
  )
...
p <- ggplot(slope_all, aes(x = Group, y = Estimate, fill = Group)) +
  ...
  scale_y_continuous(limits = c(y_min, y_max)) +
  labs(
    title = "Estimated eGFR Slopes by Group (Deaths within 6 months from time0 excluded)",
    x = NULL,
    y = "Slope (mL/min/1.73 m2 per year) with 95% CI"
  ) +
  theme_minimal(base_size = 13) +
  theme(
    legend.position = "none",
